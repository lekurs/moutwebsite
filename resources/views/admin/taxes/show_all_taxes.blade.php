            ...
            <table class="table table-striped custom-table mb-0 datatable dataTable no-footer mb-4 mt-4" id="taxes">
                ...
                @foreach($taxes as $taxe)
                    <tr>
                        ...
                        <td><a href="{{ route('taxesShowOne', $taxe->id) }}" class="txt">{{ $taxe->tax }}</a></td>
                        <td>{{ $taxe->main }}</td>
                        <td>
                            <div class="dropdown-mout dropdown-mout-table">
                                ...
                                <div class="dropdown-menu-mout dropdown-menu-mout-table dropdown-mout-status" x-placement="bottom-end" style="position: absolute; transform: translate3d(-17px, 31px, 0px); top: 0; left: 0; will-change: transform;">
                                    <a class="dropdown-item" href="{{ route('serviceStatus', $taxe->id) }}"><i class="fa fa-dot-circle-o text-success"></i> Activé</a>
                                    <a class="dropdown-item" href="{{ route('serviceStatus', $taxe->id) }}"><i class="fa fa-dot-circle-o text-danger"></i> Désactivé</a>
                                </div>
                            </div>
                        </td>
                        <td class="text-right">
                            <div class="dropdown-mout dropdown-mout-table dropdown-mout-status">
                                ...
                                <div class="dropdown-menu-mout dropdown-menu-mout-table">
                                    <a class="dropdown-item" href="{{ route('serviceEditForm', $taxe->id) }}" data-id=""><i class="fal fa-pen"></i> Modifier</a>
                                    <a class="dropdown-item" href="{{ route('taxDelete', $taxe->id) }}" data-id=""><i class="fal fa-trash"></i> Supprimer</a>
                                </div>
                            </div>
                        </td>
                    </tr>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('taxAdd') }}" method="post" enctype="multipart/form-data">
                        @csrf
